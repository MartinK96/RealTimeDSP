...
#define BUF_SIZE 960
short buffer[BUF_SIZE], buffer2[BUF_SIZE];
short input, output, delayed, outputdelayed;
int i=0; 
...
// 	input = IN_L; //read new input sample
// 	delayed = buffer[i]; //read output of delay line
// 	OUT_L = input + gain*(input-delayed); //output sum of new and delayed samples
// 	OUT_R = input + gain*(input-delayed); //output sum of new and delayed samples
...
// 	if(++i >= BUF_SIZE) i=0; //new input sample then increment 
	...
	/* Reverberation effect will be added here */
 	...
 	input = IN_L; //read new input sample
 	delayed = buffer[i]; //read output of delay line (INPUT)
 	outputdelayed= buffer2[i]; //(OUTPUT)
 	
 	OUT_L = gain*(outputdelayed)+delayed; //output sum of new and delayed samples
 	OUT_R = gain*(outputdelayed)+delayed; //output sum of new and delayed samples
